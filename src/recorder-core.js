...
(function (factory) {
	...
	*/
	...
	var Connect = function (streamStore, isUserMedia) {
		...
		//浏览器回传的音频数据处理
		var onReceive = function (channelNo, float32Arr) {
		...
		var oldScript = function () {
			...
			process.onaudioprocess = function (e) {
				...
				if (_Dset) {//临时调试用的参数，未来会被删除
					...
					setTimeout(function () { onReceive(0, arr) });//立即退出回调，试图减少对浏览器录音的影响
				} else {
					onReceive(0, arr);
				};
			};
	...
	*/
	...
	function initFn(set) {
		...
		var o = {
			...
			, channelNo: 0 //处理哪个声道，0：左声道，1：右声道
	...
	Recorder.prototype = initFn.prototype = {
		...
		, resume: function () {
			...
			if (This.state) {
				...
				stream._call[This.id] = function (pcm, sum, channelNo) {
					if (This.state == 1 && This.set.channelNo === channelNo) {
						This.envIn(pcm, sum);
					};
				};
	...
	var WebM_Extract = function (inBytes, scope) {
		...
		if (track0[sampleRateTxt] != scope[sampleRateTxt] || track0.bitDepth != 32 || track0.channels < 1 || !/(\b|_)PCM\b/i.test(track0.codec)) {
			...
			if (!scope.bad) CLog("WebM Track非预期", 3, scope, track0[sampleRateTxt]);
			scope.bad = 1;
			return -1;
		}
		...
		if (dataLen) {
			...
			if (track0.channels === 1) {
				...
				for (var c = 0; c < track0.channels; c++) {
					...
					channels.push(new Float32Array(arr2));
				}
			};

			if (!scope._debug) {
				scope._debug = 1;
				CLog("WebM Extract", track0);
			}
